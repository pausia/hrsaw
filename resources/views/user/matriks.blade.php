            ...
            <div class="flex flex-wrap -m-4 items-center">
                <div class="p-4 lg:w-2/6">
                    <div class="lg:max-w-lg mt-10 lg:w-full lg:mt-0 lg:col-span-5 lg:flex">
                        <img src="{{ asset('assets/src/illust-matriks.svg') }}" alt="mockup">
                    </div>
                </div>
                <div class="p-4 lg:w-4/6 flex flex-col items-start">
                    <h1 class="title-font sm:text-3xl leading-10 text-2xl font-bold text-gray-700">Mengoptimalkan Evaluasi Kriteria Kandidat</h1>
                    <p class="max-w-full mt-6 font-light text-gray-600 leading-relaxed lg:mb-6 md:text-lg lg:text-xl dark:text-gray-400">Manfaatkan halaman tambah matriks nilai ini untuk menyusun penilaian yang komprehensif terhadap kriteria kandidat yang diinginkan. Dengan matriks nilai, Anda dapat secara visual mengidentifikasi dan memetakan bobot relatif dari setiap nilai atau aspek yang penting dalam proses seleksi.</p>
                    <div class="self-end">
                        <img src="{{ asset('assets/src/illus-value.svg') }}" alt="mockup">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <h5 class="mr-3 font-semibold dark:text-white">Matriks Value</h5>
                    <p class="text-gray-500 dark:text-gray-400">Silahkan tambah matriks value kandidat yang anda inginkan.</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-indigo-700 uppercase dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-4">Alternatif</th>
                    ...
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            @foreach ($matriks as $matrik)
                                <th scope="col" class="px-4 py-3">C {{ $loop->index + 1 }}</th>
                            @endforeach
                    ...
                    <tbody>
                        @foreach($matriks as $matrik)
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $matrik['alternative'] }}</th>
                                @foreach ($bobot_kriteria as $criteria)
                                <td class="px-4 py-3">{{ $matrik['c' . $criteria->id] }}</td>
                                @endforeach
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="apple-imac-27-dropdown-{{ $loop->iteration }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                            <li>
                                                <a href=""><button type="submit" data-modal-target="readProductModal" data-modal-toggle="readProductModal" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
