            ...
            <div class="flex flex-wrap -m-4 items-center">
                <div class="p-4 lg:w-2/6">
                    <div class="lg:max-w-lg mt-10 lg:w-full lg:mt-0 lg:col-span-5 lg:flex">
                        <img src="{{ asset('assets/src/illust-alternatif.svg') }}" alt="mockup">
                    </div>
                </div>
                <div class="p-4 lg:w-4/6 flex flex-col items-start">
                    <h1 class="title-font sm:text-3xl leading-10 text-2xl font-bold text-gray-700">Pengelolaan Alternatif Kandidat</h1>
                    <p class="max-w-full mt-6 font-light text-gray-600 leading-relaxed lg:mb-6 md:text-lg lg:text-xl dark:text-gray-400">Optimalkan pengalaman Anda dalam menyeleksi kandidat dengan menggunakan halaman tambah alternatif ini. Dengan fitur ini, Anda dapat dengan mudah memasukkan nama-nama kandidat yang menurut Anda memiliki potensi terbaik.</p>
                    <div class="self-end">
                        <img src="{{ asset('assets/src/illus-tabelalternatif.svg') }}" alt="mockup">
                    </div>
                </div>
            </div>
        </div>
...
<section class="bg-gray-100 dark:bg-gray-900 p-3 mb-12 sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 pb-5 lg:px-4">
        <!-- Start coding here -->
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-6">
                <div class="w-full md:w-1/2">
                    <h5 class="mr-3 font-semibold dark:text-white">Alternatif Kandidat</h5>
                    <p class="text-gray-500 dark:text-gray-400">Silahkan tambah alternatif kandidat yang anda inginkan.</p>
                </div>
                    ...
                    <tbody>
                        @foreach ($alternatives as $alternative)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">{{ $alternative->name }}</td>
                                <td class="px-4 py-3">{{ $alternative->email }}</td>
                                <td class="px-4 py-3">{{ $alternative->position }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="apple-imac-27-dropdown-{{ $loop->iteration }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                            <li>
                                                <a href="{{ route('alternatif.update', $alternative->id) }}"><button type="submit" data-modal-target="readProductModal" data-modal-toggle="readProductModal" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $alternatives->firstItem() ?? '' }}
                    </span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $alternatives->lastItem() ?? '' }}
                    </span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $alternatives->total() ?? '' }}
                ...
                <div class="flex items-center space-x-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    @else
                        <a href="{{ $alternatives->previousPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                            <span class="sr-only">Previous</span>
                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                    ...
                    @if ($alternatives->hasMorePages())
                        <a href="{{ $alternatives->nextPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
            ...
            <form action="{{ route('alternatif.update', $alternative->id) }}" method="post">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ $alternative->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500" placeholder="Type product name" required="">
                    </div>
                </div>
