...
<section class="text-gray-600 body-font">
    <div  class="container px-5 pt-12 pb-5 mx-auto">
        <div class="container mx-auto md:p-14 justify-between p-5 bg-white rounded-3xl">
            <div class="flex flex-wrap -m-4 items-center">
                <div class="p-4 lg:w-2/6">
                    <div class="lg:max-w-lg mt-10 lg:w-full lg:mt-0 lg:col-span-5 lg:flex">
                        <img src="{{ asset('assets/src/illus-bobot.svg') }}" alt="mockup">
                    </div>
                </div>
                <div class="p-4 lg:w-4/6 flex flex-col items-start">
                    <h1 class="title-font sm:text-3xl leading-10 text-2xl font-bold text-gray-700">Tambahkan Penilaian Kriteria Kandidat</h1>
                    <p class="max-w-full mt-6 font-light text-gray-600 leading-relaxed lg:mb-6 md:text-lg lg:text-xl dark:text-gray-400">Manfaatkan fitur penambahan bobot atau kriteria ini untuk memberikan nilai yang lebih terperinci pada setiap aspek penting dalam proses seleksi kandidat. Dengan kemampuan menentukan bobot. <span class="text-indigo-700 font-bold">Perlu di ingat weightnya berjumlah persen jadi terhitung semua yang anda inputkan sebesar 100%</span> </p>
                    <div class="self-end">
                        <img src="{{ asset('assets/src/illus-tabelbobot.svg') }}" alt="mockup">
                    </div>
                </div>
            </div>
        </div>
...
<section class="bg-gray-100 dark:bg-gray-900 p-3 sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 pb-5 lg:px-5">
        <!-- Start coding here -->
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <h5 class="mr-3 font-semibold dark:text-white">Kriteria</h5>
                    <p class="text-gray-500 dark:text-gray-400">Silahkan tambah kriteria yang anda inginkan.</p>
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Criteria Name</th>
                            <th scope="col" class="px-4 py-3">Weight</th>
                            <th scope="col" class="px-4 py-3">Attribut</th>
                    ...
                    <tbody>
                        @foreach ($bobot_kriteria as $bobot)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">C {{ $loop->iteration }}</td>
                                <td class="px-4 py-3">{{ $bobot->criteria }}</td>
                                <td class="px-4 py-3">{{ $bobot->weight }}</td>
                                <td class="px-4 py-3">{{ $bobot->attribute }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button id="apple-imac-27-dropdown-button-{{ $loop->iteration }}" data-dropdown-toggle="apple-imac-27-dropdown-{{ $loop->iteration }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                        </svg>
                                    </button>
                                    <div id="apple-imac-27-dropdown-{{ $loop->iteration }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                            <li>
                                                <a href="{{ route('bobot.update', $bobot->id) }}"><button type="submit" data-modal-target="readProductModal" data-modal-toggle="readProductModal" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                            ...
                                            <li>
                                                <form action="{{ route('bobot.delete', $bobot->id) }}" method="post">
                                                    @csrf
                                                    @method('delete')
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $bobot_kriteria->firstItem() ?? '' }}
                    </span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $bobot_kriteria->lastItem() ?? '' }}
                    </span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ $bobot_kriteria->total() ?? '' }}
                    </span>
                </div>
                <!-- Menambahkan navigasi paginasi -->
                <div class="flex items-center space-x-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                    <!-- Tombol Previous -->
                    @if ($bobot_kriteria->onFirstPage())
                        <a href="#" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400">
                    ...
                    @else
                        <a href="{{ $bobot_kriteria->previousPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                            <span class="sr-only">Previous</span>
                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                    ...
                    <!-- Tombol Halaman -->
                    @foreach ($bobot_kriteria->getUrlRange(1, $bobot_kriteria->lastPage()) as $page => $url)
                        <a href="{{ $url }}" class="flex items-center justify-center text-sm py-2 px-3 leading-tight {{ $page == $bobot_kriteria->currentPage() ? 'text-indigo-600 bg-indigo-50 border border-indigo-300 hover:bg-indigo-100 hover:text-indigo-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white' : 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                            {{ $page }}
                        </a>
                    @endforeach

                    <!-- Tombol Next -->
                    @if ($bobot_kriteria->hasMorePages())
                        <a href="{{ $bobot_kriteria->nextPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                    ...
                    @else
                        <a href="#" class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                            <span class="sr-only">Next</span>
                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10l-3.293-3.293a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                            </svg>
            ...
            <form action="{{ route('bobot.store') }}" method="post">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="criteria" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Criteria Name</label>
                        <input type="text" name="criteria" id="criteria" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500" placeholder="Design System" required="">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="weight" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Weight</label>
                        <input type="number" name="weight" id="weight" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500" placeholder="8.2" required="">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="attribute" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Attribute</label>
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3 sm:space-x-4">
                    <button type="button" class="text-white inline-flex items-center bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-center items-center space-x-4">
                <button data-modal-toggle="deleteModal" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-indigo-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
